...
class CheeseVat:
    def __init__(self):
        ...
        self.STEP_DURATION_SEC = 15  # Each step is 15 seconds
        self.TOTAL_MILK = 10000.00  # Total milk to process (liters)
        self.MILK_FLOW_RATE = 5.00  # Liters per second
        self.MILK_PER_STEP = self.MILK_FLOW_RATE * self.STEP_DURATION_SEC  # Liters per step

        # --- Temperature Thresholds (°C) ---
        self.INITIAL_TEMP = 20.00  # Starting temperature
        self.TEMP_OPTIMAL_MIN = 31.00  # Minimum optimal temperature
        self.TEMP_OPTIMAL_MAX = 33.00  # Maximum optimal temperature
        self.TEMP_COOKING = 39.00  # Target temperature for cooking

        # --- pH Thresholds ---
        self.INITIAL_PH = 6.70  # Starting pH
        self.OPTIMAL_PH = 6.55  # Optimal pH for rennet

        # --- Base Rates (per 15-second step) ---
        self.BASE_MILK_CONVERSION_RATE = 0.02  # Base rate of milk conversion per step
        self.BASE_WHEY_RELEASE_RATE = 0.02  # Base rate of whey release during cutting/stirring
        self.BASE_WHEY_DRAIN_RATE = 0.05  # Base rate of whey draining per step

        # --- Conversion Rates (Industry Standards) ---
        self.MILK_TO_CURD_NORMAL = 0.12  # 12% milk to curd conversion rate (industry standard)
        self.MILK_TO_WHEY_NORMAL = 0.88  # 88% milk to whey conversion rate

        # --- Anomaly Settings ---
        self.DEFAULT_ANOMALY_PROBABILITY = 10  # Default 10% chance of anomaly
        ...
        self.MAX_STEPS_PER_PHASE = 1000  # Maximum steps per phase
    ...
    def run(self, anomaly_probability=None):
        """Main method that runs the cheese vat simulation (equivalent to original cheese_vat_process function)"""
        if anomaly_probability is None:
            anomaly_probability = self.DEFAULT_ANOMALY_PROBABILITY
            
        print(f"Running cheese vat simulation with {anomaly_probability}% anomaly probability...")
        env = simpy.Environment()
        env.process(self.cheese_vat_process(env, anomaly_probability))
...
# Start of cheese vat simulation
if __name__ == "__main__":
    print("Cheese Vat Simulation for Cheddar Production")
    print(f"Anomaly probability: {CheeseVat().DEFAULT_ANOMALY_PROBABILITY}%")
    print()
    vat = CheeseVat()
    vat.run()
