...
TOTAL_CHEESE = 1000  # Total cheese to process (Kilograms)
FLOW_RATE_PER_STEP = 41.7  # Liters per 15 seconds
FACTORY_TEMP = 25  # fluctuation definer per step

# --- Temperature Thresholds (°C) ---
TEMP_MIN_OPERATING = 29 #lowest temperature at which the machine would draw milk in
TEMP_OPTIMAL_MIN = 30 #minimum temp to pasteurise the milk
TEMP_OPTIMAL = 31 #optimal temp to pasteurise milk
TEMP_OPTIMAL_MAX = 31 #maximum temp to pasteurise milk
TEMP_BURN_THRESHOLD = 32 #temp at which milk would be burnt

# --- Time Durations (minutes) ---
STARTUP_DURATION = 5 #amount of minutes it would take to start the machine up
RUNTIME_LIMIT = 655 #amount of time the machine would run for (65 in total with 5 minute startup)
...
def milling_process(env, incoming_blocks, flow_per_step, initial_temp):

    # storage state variables
    cheddared_curd_blocks = 0
    milled_curd_chips = 0
    current_blocks = 0
    ...
    # Header
    print(f"{'Time':<8} {'Starting blocks ':<14} {'Chopping blocks':<14} {'Curd Chips':<14} {'Burnt (L)':<14} {'Temp (°C)':<10} Status")
    print("-" * 95)

    # --- Startup Phase ---
    for _ in range(STEPS_STARTUP):
        print(f"{format_sim_time(env.now):<8} {cheddared_curd_blocks:<14.2f} {current_blocks:<14.2f} {milled_curd_chips:<14.2f} {burnt:<14.2f} {temperature:<10.2f} Startup / Heating")
        yield env.timeout(1)

    # --- Processing Loop ---
    while (cheddared_curd_blocks + current_blocks > 0) and (milled_curd_chips + burnt < total_milk):

        # Scheduled cooldown
        if cooldown_scheduled:
            print(f"{format_sim_time(env.now):<8} {cheddared_curd_blocks:<14.2f} {current_blocks:<14.2f} {milled_curd_chips:<14.2f} {burnt:<14.2f} {temperature:<10.2f} Scheduled Cooldown")
        ...
        if cooling_due_to_overheat:
            ...
            print(f"{format_sim_time(env.now):<8} {cheddared_curd_blocks:<14.2f} {current_blocks:<14.2f} {milled_curd_chips:<14.2f} {burnt:<14.2f} {temperature:<10.2f} Cooling after Overheat")
            yield env.timeout(1)
            run_time_steps += 1

            if temperature <= TEMP_OPTIMAL:
                cooling_due_to_overheat = False
                print(f"{format_sim_time(env.now):<8} {cheddared_curd_blocks:<14.2f} {current_blocks:<14.2f} {milled_curd_chips:<14.2f} {burnt:<14.2f} {temperature:<10.2f} Cooled - Resuming")
            continue

        # Choose tank source
        if cheddared_curd_blocks > 0:
            source = "Start Tank"
            available = cheddared_curd_blocks
        elif current_blocks > 0:
            source = "Balance Tank"
            available = current_blocks
        else:
            break
        ...
        if temperature >= TEMP_BURN_THRESHOLD:
            ...
            if source == "Start Tank":
                cheddared_curd_blocks -= milk_this_step
            else:
                current_blocks -= milk_this_step
            ...
            milled_curd_chips += milk_this_step
            status = f"milled_curd_chips from {source}"
        ...
        print(f"{format_sim_time(env.now):<8} {cheddared_curd_blocks:<14.2f} {current_blocks:<14.2f} {milled_curd_chips:<14.2f} {burnt:<14.2f} {temperature:<10.2f} {status}")
    ...
    # --- Final Report ---
    total_out = milled_curd_chips + burnt
    print("\n--- Simulation Complete ---")
    print(f"Remaining - Cheddared Block count: {cheddared_curd_blocks}")
    print(f"Remaining - Curd Chip count: {milled_curd_chips}")

def run_milling_sim():
    """Initialize and run the simulation."""
    env = simpy.Environment()
    env.process(milling_process(env, TOTAL_CHEESE, FLOW_RATE_PER_STEP, TEMP_OPTIMAL))
    env.run(until=500)
